...
class wo_inf_resumen_venta extends w_informe_pantalla {
   ...
   function wo_inf_resumen_venta() {
		...
		$sql = "select COD_INF_RESUMEN_VENTA
					  ...
					  ,CASE  
                        WHEN COD_EMPRESA = -11 THEN 'NEGRITA'
						WHEN COD_EMPRESA = -12 THEN 'NEGRITA'
						WHEN COD_EMPRESA = -13 THEN 'NEGRITA'
					  END CLASS_NEGRITA
		...
		//titulos
		$sql = "select 'CLIENTES DESTACADOS'		TIT_EMPRESA
		...
		// totales
		$sql = "select 'TOTALES (INTERNOS + OTROS + ARRIENDO)'				LABEL
					  ,sum(MONTO_ANO)		VENTA_ANT
					  ,sum(MONTO_ANO_II)	VENTA_ACT
				  	  ,1					ORDEN
				  	  ,'bold'					LAST_BOLD
					  from INF_RESUMEN_VENTA
				where COD_EMPRESA in (-11, -12, -13)	--no arriendo";
				//union								
				...
				//  	  ,''					LAST_BOLD
				//from INF_RESUMEN_VENTA
				//where COD_EMPRESA in (-5, -6)		--si arriendo
				//union				
				//select 'Total'				LABEL
				//	   ,sum(MONTO_ANO)		VENTA_ANT
				//	  ,sum(MONTO_ANO_II)	VENTA_ACT
