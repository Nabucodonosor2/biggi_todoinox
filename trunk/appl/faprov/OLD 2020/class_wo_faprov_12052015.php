	...
	const K_PARAM_DIRECTORIO = 31;
   	...
   	function wo_faprov() {
		...
		if ($priv=='E') {
			...
			$this->dw->add_control(new edit_check_box('SELECCION_CAMBIO', 'S', 'N'));
      	}
      	else {
			$this->dw->add_control(new control_null('SELECCION'));
			$this->dw->add_control(new control_null('SELECCION_CAMBIO'));
      	}
...
//				echo $result[0]['SALDO'];
	...
	function cambio_estado() {
		...
		$i = 0;
		while (($i < $this->row_per_page) && ($ind < $this->row_count_output)){
			$seleccion = $this->dw->get_item($i, 'SELECCION_CAMBIO');
			if ($seleccion=='S') { 
				$cod_faprov = $this->dw->get_item($i, 'COD_FAPROV');
