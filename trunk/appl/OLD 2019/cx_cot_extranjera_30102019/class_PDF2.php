...
private $pagebreak = false;
	...
	static function makePDF($t,$d,$rd, $con_logo=true, $orientation='P',$unit='pt',$format='letter'){
		...
		$pdf=new PDF2($t,$d,$rd, $con_logo, $orientation,$unit,$format);
		return $pdf;
	}
	function PDF2($t,$d,$rd, $con_logo, $orientation,$unit,$format) {
	...
	function Header(){
		if ($this->con_logo) {
			$file_name = session::get('K_ROOT_DIR').'images_appl/'.session::get('K_CLIENTE').'/cx_logo_reporte.jpg';
			if (file_exists($file_name)) 
				$this->Image($file_name, 0, 0,612,792);
			else
				$this->Image(session::get('K_ROOT_DIR').'images_appl/cx_logo_reporte.jpg', 0, 0,612,792);
		}
