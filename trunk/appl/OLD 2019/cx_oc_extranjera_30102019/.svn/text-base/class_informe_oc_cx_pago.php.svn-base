...
class informe_oc_cx_pago extends reporte {	
	...
	function modifica_pdf(&$pdf) {
		...
		$pdf->Cell(47, 15, 'Estimado '.$result[0]['NOM_ATENCION_CORTO'].' :' , 0 , 0, 'L');
		
		$pdf->SetXY(52, $y+234+15);
		$pdf->MultiCell(450, 14, 'Por la presente autorizo debitar de cta. cte. US$ N°01-117326-0, COMERCIAL TODOINOX LTDA., (Rut: 89.257.000-0) la suma de US$ '.$result[0]['MONTO_PAGO'].' y enviar transferencia por pago anticipado de importacion a:' , 0 , 'J', false);
		...
		$pdf->MultiCell(400, 14, 'Los gastos que genere esta operación cargarlos a nuestra cta. cte. nro. 01-125086 Favor enviarnos copia de Swift a mail '.$result[0]['MAIL'].'.' , 0 , 'J', false);
