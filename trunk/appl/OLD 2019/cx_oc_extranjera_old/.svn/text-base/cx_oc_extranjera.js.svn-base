function dlg_print(){
	...
	var returnVal = window.showModalDialog("dlg_print_oc_extranjera.php", "_blank", args);
 	if (returnVal == null)
 		return false;
	else{
...
function dlg_crea_desde_oc(){
	var vl_args = "location:no;dialogLeft:400px;dialogTop:200px;dialogWidth:450px;dialogHeight:180px;dialogLocation:0;Toolbar:no;";
	var vl_returnVal = window.showModalDialog("dlg_crea_desde_oc.php", "_blank", vl_args);
 	if (vl_returnVal == null)
 		return false;
	else{
		document.getElementById('wo_hidden').value = vl_returnVal;
   		return true;
	}
}