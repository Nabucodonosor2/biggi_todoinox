ALTER PROCEDURE [dbo].[spu_usuario](@ve_operacion varchar(20)
									...
									,@ve_plataforma_print_cheque varchar(100)=null)
AS
BEGIN
    ...
    if (@ve_operacion='UPDATE') begin
		...
		IF (@ve_password = '4b197ae8c766311824f077e3885d3478') 
			...
			plataforma_print_cheque = @ve_plataforma_print_cheque
			...
			plataforma_print_cheque = @ve_plataforma_print_cheque
			where cod_usuario = @ve_cod_usuario
		/*solo si es ANGEL SCIANCA, se le cambia la contraseña en los 4 sistemas al mismo tiempo */ 
		if(@ve_cod_usuario=16)begin
			/*BIGGI*/
			update BIGGI.dbo.USUARIO
			set PASSWORD=@ve_password
				,FECHA_PASSWORD = GETDATE()
			where COD_USUARIO = 2;
			/*BODEGA_BIGGI*/
			update BODEGA_BIGGI.dbo.USUARIO
			set PASSWORD=@ve_password
				,FECHA_PASSWORD = GETDATE()
			where COD_USUARIO = 9;
			/*RENTAL*/
			update RENTAL.dbo.USUARIO
			set PASSWORD=@ve_password
				,FECHA_PASSWORD = GETDATE()
			where COD_USUARIO = 13;
			/*TODOINOX*/
			update TODOINOX.dbo.USUARIO
			set PASSWORD=@ve_password
				,FECHA_PASSWORD = GETDATE()
