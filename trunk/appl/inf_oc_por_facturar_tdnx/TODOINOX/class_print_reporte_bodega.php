...
class OC_PDF_bodega extends PDF {
	var $posY_subtotal = 0;
	var $barcode_dibujado = false;
	var $sql_print = "";
	
	
	function OC_PDF_bodega($t,$d,$rd, $con_logo, $orientation,$unit,$format,$sql){
		$this->sql_print = $sql;
		session::set("K_CLIENTE", "BODEGA");
...
class print_reporte_bodega extends reporte{	
	function print_reporte_bodega($sql, $xml, $labels=array(), $titulo, $con_logo, $print_especial=false) {
		if (!$print_especial) {
	...
	function make_reporte() {
		...
		$pdf = new OC_PDF_bodega(array($p), array($this->labels), array($rdata), $this->con_logo,$this->orientation,$this->unit,$this->format,$this->sql);
