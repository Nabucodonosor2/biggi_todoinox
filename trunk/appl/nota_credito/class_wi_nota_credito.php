    ...
    function enviar_dte($reenviar = false){
        if (!$this->lock_record())
            ...
            if($result[0]['NRO_NOTA_CREDITO'] <> '' && $reenviar == false)
				...
				//MH 01062022 SE MODIFICA PARA QUE VALIDE SOLO DESDE EL COD_FACTURA 93595
				//MH 01062022 DESDE EL 31052022 AL CERRAR BIGGI SE DEJA DE USAR LIBRE DTE INTEGRASYSTEM Y SE PASA A LIBRE DTE BY MH                
                if($revision_estado <> 'EPR' && $revision_estado <> 'RPR' && $revision_estado <> 'RLV' && $COD_FACTURA > 93595){//ultima factura signature cod_factura 52176
                    $this->_load_record();
