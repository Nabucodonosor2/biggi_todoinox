				...
				begin
					...
					if (@vl_nro_nota_credito is null)
						begin
							...
							select @vl_cod_empresa = nc.cod_empresa
								  ,@vl_cod_doc = f.nro_factura
							from nota_credito nc
								,factura f
							where cod_nota_credito = @ve_cod_nota_credito
							and f.cod_factura = nc.cod_doc
							
							if(@vl_cod_empresa <> 1 AND @vl_cod_empresa <> 37)begin
								exec spu_nota_credito 'CREA_INGRESO_PAGO', @ve_cod_nota_credito
							end
							else begin
								if(@vl_cod_empresa = 1)BEGIN
									if(@vl_cod_doc < 128800)
										exec spu_nota_credito 'CREA_INGRESO_PAGO', @ve_cod_nota_credito
								END
							end
