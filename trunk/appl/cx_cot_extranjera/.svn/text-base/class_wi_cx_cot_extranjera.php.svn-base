...
class wi_cx_cot_extranjera extends w_input {
	function wi_cx_cot_extranjera($cod_item_menu) {
		...
		$this->dws['wi_cx_cot_extranjera']->add_control(new edit_text('REFERENCIA',140, 200));
	...
	function print_record(){
		...
		$rpt = new informe_cot_extranjera($sql, $file_name, $labels, "Cotización Extranjera".".pdf", 1);												
		$this->_load_record();
	}
}
?>