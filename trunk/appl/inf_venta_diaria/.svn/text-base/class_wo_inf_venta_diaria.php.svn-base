...
class dw_docs extends datawindow {	
	function dw_docs($fecha1, $fecha2,$no_incluye_relacionado) {
		$sql = "select TDP.NOM_TIPO_DOC_PAGO
				...
				from FACTURA F, INGRESO_PAGO_FACTURA IPF, DOC_INGRESO_PAGO DIP, TIPO_DOC_PAGO TDP, MONTO_DOC_ASIGNADO MDA
				where F.FECHA_FACTURA between $fecha1 and $fecha2
				  and ('$no_incluye_relacionado'='N' or F.COD_EMPRESA not in (1, 37,44,38))	/*COMERCIAL,BODEGA_BIGGI,CATERING,SERVINDUS*/
...
class wo_inf_venta_diaria extends w_informe_pantalla {
	...
	function print_informe() {
		...
		$rpt = new print_venta_diaria($sql, $sql_dw2, $xml, $labels, "Ventas Diaria", 'logo');
		$this->_redraw();
	}
}
