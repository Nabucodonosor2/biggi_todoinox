...
class reporte_biggi extends reporte {	
	...
	const K_PARAM_NOM_UNIVERSIDAD	= 5;
	const K_PARAM_DIRECCCION 		= 9;
	const K_PARAM_COMUNA 			= 11;
	const K_PARAM_CIUDAD 			= 12;
	const K_PARAM_PAIS 				= 16;
	const K_PARAM_TELEFONO			= 10;
	const K_PARAM_FAX 				= 15;
	const K_PARAM_E_MAIL 			= 13;
	const K_PARAM_SITIO_WEB 		= 14;
	...
	function reporte_biggi($sql, $xml, $labels=array(), $titulo, $con_logo, $vuelve_a_presentacion=false, $sp='', $param='',$orientation='P',$unit='pt',$format='letter') {
		$this->sp = $sp;
		$this->param = $param;
		$sql_param = "select convert(varchar, getdate(), 103) FECHA";
		
		$db = new database(K_TIPO_BD, K_SERVER, K_BD, K_USER, K_PASS);
		$result = $db->build_results($sql_param);
		
		$labels['str_fecha'] = $result[0]['FECHA'];
