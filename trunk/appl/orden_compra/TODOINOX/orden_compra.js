...
function validate() {
	...
	*/
	var es_inventario = document.getElementById('ES_INVENTARIO_0').checked;
	if(es_inventario == true){
	
		for (var i=0; i<aTR.length; i++) {
			var vl_record = get_num_rec_field(aTR[i].id);
			var vl_cod_producto = document.getElementById('COD_PRODUCTO_' + vl_record).value;
			if(vl_cod_producto == 'TE' | vl_cod_producto == 'F' | vl_cod_producto == 'E' | vl_cod_producto == 'I'){
				alert("No se puede ingresa el producto '"+ vl_cod_producto +"'");
				return false;
			}
		}
	}
