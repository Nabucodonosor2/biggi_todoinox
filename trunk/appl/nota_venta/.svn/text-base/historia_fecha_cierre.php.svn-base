...
$temp = new Template_appl('historia_fecha_cierre.htm');	
$db = new database(K_TIPO_BD, K_SERVER, K_BD, K_USER, K_PASS);

	// Historial de modificaciones de plazo cierre NV
	$sql = "select U.NOM_USUARIO,
			...
			where LG.NOM_TABLA = 'NOTA_VENTA' and
				...
				DC.NOM_CAMPO = 'FECHA_PLAZO_CIERRE' 
	...
	for ($i=0 ; $i <count($result); $i++) {
		...
		$temp->setVar("DETALLE_CAMBIO.VALOR_ANTIGUO", $valor_old);
		
		$valor_new = $result[$i]['VALOR_NUEVO'];
		$temp->setVar("DETALLE_CAMBIO.VALOR_NUEVO", $valor_new);						
	}
