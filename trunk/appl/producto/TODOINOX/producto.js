...
function redondeo_biggi() {
	...
	const vl_precio_add = get_value('PRECIO_ADICIONAL_0');
	if(vl_precio_add == '')
		vl_precio_add == 0;
	...
	precio_vta_sug = precio_vta_sug + parseInt(to_num(vl_precio_add));

	set_value('PRECIO_TOTAL_SUG_0', number_format(precio_vta_sug, 0, ',', '.'), number_format(precio_vta_sug, 0, ',', '.'));
...
function valida_precio_venta_pub(){
	...
	if(precio_vta_pub_sugerido > ve_pre_vta_pub){
		if(!confirm("Está seguro que deséa ingresar un Precio Público más bajo que el sugerido?\n\nSugerido = $"+number_format(precio_vta_pub_sugerido, 0, ',', '.')+"\n\nIngresado por ud = $"+number_format(ve_pre_vta_pub, 0, ',', '.'))){
			document.getElementById('PRECIO_VENTA_PUBLICO_0').value = document.getElementById('PRECIO_VENTA_PUBLICO_NO_ING_0').innerHTML.replace(".","");
			calc_precio_int_pub();
			return;
		}
	}
...
function calc_precio_int_pub(ve_alerta = 'N') {	
	/**	
	...
	**/
	...
	var pre_vta_int_ni = document.getElementById('PRECIO_VENTA_INTERNO_NO_ING2_0');
	...
	if(precio_vta_int_sugerido > ve_pre_vta_int && ve_alerta == 'N'){
		if(!confirm("Está seguro que deséa ingresar un Precio Interno más bajo que el sugerido?\n\nSugerido = $"+number_format(precio_vta_int_sugerido, 0, ',', '.')+"\n\nIngresado por ud = $"+number_format(ve_pre_vta_int, 0, ',', '.'))){
			document.getElementById('PRECIO_VENTA_INTERNO_0').value = document.getElementById('PRECIO_VENTA_INTERNO_NO_ING_0').innerHTML.replace(".","");
			calc_precio_int_pub('S');
			return;
		}	
	}
...
function actualiza_otros_tabs() {
	if(document.getElementById('COD_PRODUCTO_PRINCIPAL_0')){
		...
		for (var i=1; i <= 5; i++) {
			if(document.getElementById('cod_producto'+i)){
...
function cambio_precio_interno(monto_int) {
  document.getElementById('PRECIO_VENTA_INTERNO_UNO_0').value = to_num(monto_int.value);
	document.getElementById('PRECIO_VENTA_INTERNO_0').value = monto_int.value;
 document.getElementById('PRECIO_VENTA_INTERNO_EC_0').innerHTML = number_format(monto_int.value, 0, ',', '.');
 
 
 var ve_pre_vta_int = to_num(monto_int.value);
	var pre_vta_int_ni = document.getElementById('PRECIO_VENTA_INTERNO_EC_0');
	var ve_fac_vta_pub = to_num(document.getElementById('FACTOR_VENTA_PUBLICO_0').value);	
	//var precio_vta_int_sugerido = document.getElementById('PRECIO_VENTA_INT_SUG_0');
	var precio_vta_pub_sugerido = document.getElementById('PRECIO_VENTA_PUB_SUG_EC_0');
...
function cambio_precio_publico(monto_vent) {
  document.getElementById('PRECIO_VENTA_PUBLICO_UNO_0').value = monto_vent.value;
	document.getElementById('PRECIO_VENTA_PUBLICO_0').value = monto_vent.value;
}
