...
function save_direccion() {
	...
	$COD_CHEQUE_TODOINOX = ($COD_CHEQUE_TODOINOX=='') ? "null" : $COD_CHEQUE_TODOINOX;
	...
	if ($db->EXECUTE_SP($sp, $param)){
		...
		$COD_CHEQUE_TODOINOX = $db->GET_IDENTITY();
			return true;
	}
...
$temp = new Template_appl('cheque_todoinox.htm');	
...
if (isset($_POST['b_enviar'])){
	if(save_direccion()){
		...
		$sql = "SELECT TOP 1 COD_CHEQUE_TODOINOX 
				...
				ORDER BY COD_CHEQUE_TODOINOX DESC";
		$result = $db->build_results($sql);
		$COD_CHEQUE_TODOINOX	= $result[0]['COD_CHEQUE_TODOINOX'];
		
		print " <script>window.open('class_reverso_ch_todoinox.php?token=$COD_CHEQUE_TODOINOX','')</script>";
