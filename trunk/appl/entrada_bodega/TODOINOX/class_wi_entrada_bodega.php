...
require_once(dirname(__FILE__)."/../../common_appl/class_reporte_biggi.php");
	...
	class dw_item_entrada_bodega extends dw_item_entrada_bodega_base {
		...
		$jsc = $this->controls['COD_PRODUCTO']->get_onChange();
		$jsn = $this->controls['NOM_PRODUCTO']->get_onChange();
		
		$this->controls['COD_PRODUCTO']->set_onChange($jsc.'show_msg_ajuste(this);');
		$this->controls['NOM_PRODUCTO']->set_onChange($jsn.'show_msg_ajuste(this);');
...
class wi_entrada_bodega extends wi_entrada_bodega_base{
	...
	function save_record($db) {
		...
		$tipo_doc = $tipo_doc=='' ? 'AJUSTE' : $tipo_doc; 
	...
	function creada_desde_ri($nro_registro_ingreso) {
		...
		$this->dws['dw_entrada_bodega']->set_item(0, 'TIPO_DOC', 'REGISTRO_INGRESO');
		$this->dws['dw_entrada_bodega']->set_item(0, 'COD_DOC', $nro_registro_ingreso);
		...
		$this->dws['dw_entrada_bodega']->set_item(0, 'REFERENCIA', $result[0]['REFERENCIA']);
		$this->dws['dw_entrada_bodega']->set_item(0, 'OBS', $result[0]['OBS']);
	}
	...
	function creada_desde_oc($cod_oc) {
	  ...
	  $sql_desde_item_oc="SELECT IOC.ITEM
		...
		$this->dws['dw_entrada_bodega']->set_item(0, 'COD_DOC', $cod_oc);
