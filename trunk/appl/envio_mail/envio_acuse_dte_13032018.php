...
for($i=0; $i < count($result); $i++) {
	...
	if($tipodte == 33 || $tipodte == 34){
		...
		if($nro_factura_bd <> $NRO_DTE)
			$body_diff = arma_body_diff($body_diff, "FA", "NRO_FACTURA", $nro_factura_bd, $NRO_DTE);
		if($rut_bd <> $rut)
			$body_diff = arma_body_diff($body_diff, "FA", "RUT", $rut_bd, $rut);
		if($total_neto_bd <> $total_neto)
			$body_diff = arma_body_diff($body_diff, "FA", "TOTAL_NETO", $total_neto_bd, $total_neto);
		if($total_con_iva_bd <> $monto_v)
			$body_diff = arma_body_diff($body_diff, "FA", "TOTAL_CON_IVA", $total_con_iva_bd, $monto_v);
		if($tipodte == 33){
			if($monto_iva_bd <> $monto_iva)
				$body_diff = arma_body_diff($body_diff, "FA", "MONTO_IVA", $monto_iva_bd, $monto_iva);
		}
		if($fecha_factura_bd <> $fecha_documento)
			$body_diff = arma_body_diff($body_diff, "FA", "FECHA_FACTURA", $fecha_factura_bd, $fecha_documento);
		//////////////////////
	}else if($tipodte == 52){
		...
		$NOM_EMPRESA_DETINATARIO	= $xml_resolucion->SetDTE->DTE->Documento->Encabezado->Receptor->RznSocRecep;
		$monto						= $xml_resolucion->SetDTE->DTE->Documento->Encabezado->Totales->MntNeto;
		...
		if($nro_guia_despacho_bd <> $NRO_DTE)
			$body_diff = arma_body_diff($body_diff, "GD", "NRO_GUIA_DESPACHO", $nro_guia_despacho_bd, $NRO_DTE);
		if($rut_bd <> $rut)
			$body_diff = arma_body_diff($body_diff, "GD", "RUT", $rut_bd, $rut);
		if($fecha_guia_despacho_bd <> $fecha_documento)
			$body_diff = arma_body_diff($body_diff, "GD", "FECHA_GUIA_DESPACHO", $fecha_guia_despacho_bd, $fecha_documento);
		///////////////////////
		...
		if($nro_nota_credito_bd <> $NRO_DTE){
			$body_diff = arma_body_diff($body_diff, "NC", "NRO_NORA_CREDITO", $nro_nota_credito_bd, $NRO_DTE);
		}
		if($rut_bd <> $rut){
			$body_diff = arma_body_diff($body_diff, "NC", "RUT", $rut_bd, $rut);
		}	
		if($total_neto_bd <> $total_neto){
			$body_diff = arma_body_diff($body_diff, "NC", "TOTAL_NETO", $total_neto_bd, $total_neto);
		}	
		if($total_con_iva_bd <> $monto_v){
			$body_diff = arma_body_diff($body_diff, "NC", "TOTAL_CON_IVA", $total_con_iva_bd, $monto_v);
		}
		if($monto_iva <> 0){
			if($monto_iva_bd <> $monto_iva){
				$body_diff = arma_body_diff($body_diff, "NC", "MONTO_IVA", $monto_iva_bd, $monto_iva);
			}
		}
		if($fecha_nota_credito_bd <> $fecha_documento)
			$body_diff = arma_body_diff($body_diff, "NC", "FECHA_NOTA_CREDITO", $fecha_nota_credito_bd, $fecha_documento);
		//////////////////
	}
...
function arma_body_diff($body_diff, $tipo_doc, $tipo_diff, $bd_value, $xml_value){
	return $body_diff."Tipo doc: $tipo_doc - Diferencia: $tipo_diff - bd_value: $bd_value xml_value:$xml_value <br><br>";
}
?>