...
class wi_inf_tdnx_inventario_valorizado_new extends w_param_informe_biggi {
	
	function __construct($cod_item_menu) {
		$xml = session::get('K_ROOT_DIR').'appl/inf_tdnx_inventario_valorizado_new/inf_tdnx_inv_valor_detalle.xml';
		parent::w_param_informe_biggi('inf_tdnx_inventario_valorizado_new',$cod_item_menu,'Inventario Valorizado.pdf',$xml,'','spi_tdnx_inv_valor_detalle');
	...
	function make_filtro() {
		...
		if($resumen =='RESUMEN'){
			...
			$xml = session::get('K_ROOT_DIR').'appl/inf_tdnx_inventario_valorizado_new/inf_tdnx_inv_valor_resumen.xml';
			$this->xml = $xml;
			$this->param = "$cod_bodega, $fecha";
		}
		else if($resumen =='DETALLE'){
			$this->sp = 'spi_tdnx_inv_valor_detalle';
			$xml = session::get('K_ROOT_DIR').'appl/<inf_tdnx_inventario_valorizado_new></inf_tdnx_inventario_valorizado_new>/inf_tdnx_inv_valor_detalle.xml';					
	...
	function genera_pdf($labels = array(), $con_logo = true,$orientation='P',$unit='pt',$format='letter') {
		...
		if($resumen =='RESUMEN') {
		    ...
		    $arrayParam = explode(",",$this->param);
		    $fecha = $arrayParam[1];
		    $db = new database(K_TIPO_BD, K_SERVER, K_BD, K_USER, K_PASS);
            $sql = "select convert(varchar,$fecha,103)FECHA";
		    $result = $db->build_results($sql);
		    $fecha = $result[0]['FECHA'];
		    $labels['strFecha'] = "Al $fecha";	
