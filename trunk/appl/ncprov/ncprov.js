...
function ajax_valida_nro_doc(){
	...
	var vl_cod_empresa	= document.getElementById('COD_EMPRESA_H_0').value;
	
	if(vl_nro_doc != ""){
		var ajax = nuevoAjax();
		ajax.open("GET", "ajax_valida_nro_doc.php?ve_nro_doc="+vl_nro_doc+"&ve_cod_empresa="+vl_cod_empresa, false);
		ajax.send(null);
		var resp = ajax.responseText;
		
		if(parseInt(resp) > 0){
			alert('El número documento ya esta siendo utilizada por otra NC Proveedor para la misma empresa en estado no anulada.');
...
function select_1_empresa(valores, record){
	set_value('COD_EMPRESA_H_' + record, valores[1], valores[1]);
	set_value('NRO_NCPROV_' + record, "", "");
	set_values_empresa(valores, record);
}