function dlg_agrega_comision(){
	...
	var url = "dlg_agrega_comision.php";
	$.showModalDialog({
		 url: url,
		 dialogArguments: '',
		 height: 220,
...
function calculo_porc_comision(){
	var vl_porc_comision_otros	= findAndReplace(get_value('PORC_COMISION_OTROS_0'), ',', '.');
	var vl_total_otros			= findAndReplace(get_value('TOTAL_OTROS_S_0'), '.', '');
	var vl_total_comision		= findAndReplace(get_value('TOTAL_COMISION_0'), '.', '');
