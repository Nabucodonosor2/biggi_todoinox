...
class wi_registro_ingreso extends w_input {
	...
	function wi_registro_ingreso($cod_item_menu) {
		...
		$this->dws['wi_registro_ingreso']->add_control(new edit_text_upper('REFERENCIA',163,100));
...
class print_registro_ingreso extends reporte {	
	...
	function modifica_pdf(&$pdf) {
		...
		if($i < 11){
			...
			while ($pdf->GetStringWidth($PRODUCTO) > 39 AND $pdf->GetStringWidth($MODELO) > 9) {
					$PRODUCTO = substr($PRODUCTO, 0, 39);
