...
class dw_item_registro_ingreso extends dw_item {
	...
	function update($db)	{
		...
		for ($i = 0; $i < $this->row_count(); $i++){
			...
			IF($COD_PRODUCTO == 'TE')
			     $DESCRIPCION_TE		 		= ($NOM_PRODUCTO =='') ? "null" : "'".$NOM_PRODUCTO."'";
			ELSE
			     $DESCRIPCION_TE = "NULL";
...
class wi_registro_ingreso extends w_input {
   ...
   function habilita_boton(&$temp, $boton, $habilita) {
        ...
        if($boton == 'referencia'){
            if($habilita){
                $control = '<input name="b_referencia" id="b_referencia" src="../../images_appl/b_referencia.jpg" type="image" '.
                    'onMouseDown="MM_swapImage(\'b_referencia\',\'\',\'../../images_appl/b_referencia_click.jpg\',1)" '.
                    'onMouseUp="MM_swapImgRestore()" onMouseOut="MM_swapImgRestore()" '.
                    'onMouseOver="MM_swapImage(\'b_referencia\',\'\',\'../../images_appl/b_referencia_over.jpg\',1)"
							 onClick="var vl_tab = document.getElementById(\'wi_current_tab_page\'); if (TabbedPanels1 && vl_tab) vl_tab.value =TabbedPanels1.getCurrentTabIndex();
									 if (document.getElementById(\'b_save\')) {
										 if (validate_save()) {
										 		document.getElementById(\'wi_hidden\').value = \'save_referencia\';
                ...
                $control = '<img src="../../images_appl/b_referencia_d.jpg">';
            }
            
            $temp->setVar("WSWAP_REFERENCIA", $control);
        }
    ...
    function procesa_event(){
        if(isset($_POST['b_save_x'])) {
            ...
            if ($this->_save_record()) {
                if ($_POST['wi_hidden']=='save_referencia')		// Es es el save enviar_dte
                    print '<script type="text/javascript"> document.getElementById(\'b_referencia\').click(); </script>';
            }
        }
        else if(isset($_POST['b_referencia_x'])){
            $this->referencia();
          }else
    ...
    function referencia(){
     ...
     $cod_registro_ingreso  = $this->get_key();
      ...
      $sp = 'spu_registro_ingreso';                        
      $param = "'REFERENCIA',null,null,null,null,null,$cod_registro_ingreso";	
    	if (!$db->EXECUTE_SP($sp, $param)){
					$this->_load_record();
          return false;
          //$this->_load_record();
				...
				$this->_load_record();
        $this->alert('Cambio de referencia exitoso');
        return true;
      }
      //
	...
	function modifica_pdf(&$pdf) {
		...
		if($count > 11){
			...
			while ($pdf->GetStringWidth($PRODUCTO) > 35 AND $pdf->GetStringWidth($MODELO) > 20) {
				...
				$pdf->MultiCell(57, 15, $detalle = number_format($CU_US,2,',','.'), '0', 'R');
				$pdf->SetXY(526, $Y);//451
				$pdf->MultiCell(58, 15, $detalle = number_format($CU_PESOS,0,',','.'), '0', 'R');
	           	$e++;
