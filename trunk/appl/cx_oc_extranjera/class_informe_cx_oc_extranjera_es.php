...
class informe_oc_extranjera_es extends reporte {	
	...
	function dibuja_uno(&$pdf, $result){
		...
		$pdf->SetXY(10, 60);
		$pdf->Cell(570, 15, 'TRADUCCIÓN / DESCRIPCIÓN MERCADERÍA.' , 0 , 0, 'C');
		$pdf->SetXY(302, 60);
		...
		$pdf->SetXY($x+480, $y+170);
		$pdf->SetFont('Arial','',9);
		//$pdf->MultiCell(67, 15, $result['NOM_PAIS_4D'] , 0 , 'L');
		$pdf->SetFont('Arial','B',9);
		...
		$pdf->SetXY($x+140, $y+220);
		$pdf->SetFont('Arial','',9);
		//$pdf->MultiCell(147, 15, $result['NOM_CX_PUERTO_SALIDA'], 0 , 'L');
		//$pdf->Line(140,235,290,235);
		$pdf->SetXY($x+375, $y+220);
		$pdf->SetFont('Arial','B',9);
		...
		foreach($result2 as $row){
			...
			//$pdf->SetXY($x+119, $y);
			$pdf->SetXY($x+59,	$y);
			$pdf->Cell(80,  $y_despues - $y_antes,$row['ITEM'], 1 , 1, 'L');
			$pdf->SetXY($x+448, $y);
		...
		////////INI FIRMA////////
		$pdf->Line(435,$ypos+90,570,$ypos+90);
