CREATE PROCEDURE [dbo].[spu_cx_item_oc_extranjera](@ve_operacion					VARCHAR(20),
    											   @ve_cod_cx_item_oc_extranjera	NUMERIC(10)=NULL,
    											   @ve_cod_cx_oc_extranjera			NUMERIC(10)=NULL,
...
BEGIN
	IF (@ve_operacion='INSERT') BEGIN
		INSERT INTO CX_ITEM_OC_EXTRANJERA VALUES(@ve_cod_cx_oc_extranjera
												,@ve_orden
												,@ve_item
												,@ve_cod_producto
												,@ve_nom_producto
	...
	IF (@ve_operacion='UPDATE') BEGIN
		UPDATE CX_ITEM_OC_EXTRANJERA
		SET COD_CX_OC_EXTRANJERA	= @ve_cod_cx_oc_extranjera,
			ORDEN					= @ve_orden,
			ITEM					= @ve_item,
			...
			PRECIO					= @ve_precio
		WHERE COD_CX_ITEM_OC_EXTRANJERA = @ve_cod_cx_item_oc_extranjera
	END
	IF (@ve_operacion='DELETE') BEGIN
		DELETE CX_ITEM_OC_EXTRANJERA
		WHERE COD_CX_ITEM_OC_EXTRANJERA = @ve_cod_cx_item_oc_extranjera
	END		      
END