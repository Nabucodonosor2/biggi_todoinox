...
class dw_persona_dos extends datawindow {
	public	$cod_empresa = null;
	
	function dw_persona_dos() {		
		$sql = "select 	P.COD_PERSONA		COD_PERSONA_DOS, 
						P.COD_SUCURSAL		P_COD_SUCURSAL_DOS, 
						P.NOM_PERSONA		NOM_PERSONA_DOS, 
						P.COD_CARGO			COD_CARGO_DOS, 
						P.TELEFONO 			TELEFONO_PERSONA_DOS,   
						P.FAX				FAX_PERSONA_DOS,
						P.EMAIL				EMAIL_DOS, 
						S.COD_EMPRESA		COD_EMPRESA_DOS,
						CASE
							WHEN ES_VIGENTE = 'S' THEN ''
							ELSE 'none'
						END TR_VIGENTE_VISIBLE
						,ES_VIGENTE
		...
		parent::datawindow($sql, 'PERSONA_DOS', true, true);	
		
		$this->add_control(new edit_text('COD_PERSONA_DOS', 10, 10, 'hidden'));
		$this->add_control($control = new edit_text_upper('NOM_PERSONA_DOS', 35, 100));
		$control->set_onChange("change_nom_persona_dos(this.id)");
		...
		$this->add_control(new drop_down_dw('COD_CARGO_DOS', $sql, 170));
		...
		$this->add_control(new drop_down_dw('P_COD_SUCURSAL_DOS', $sql, 147));
		$this->add_control(new edit_text_upper('TELEFONO_PERSONA_DOS', 15, 100));	
		$this->add_control(new edit_text_upper('FAX_PERSONA_DOS', 15, 100));
		$this->add_control(new edit_mail('EMAIL_DOS', 35, 100));
		$this->add_control(new edit_check_box('ES_VIGENTE', 'S', 'N'));
		
		// asigna los mandatorys	
		$this->set_mandatory('P_COD_SUCURSAL_DOS', 'Sucursal de la Persona');
		$this->set_mandatory('NOM_PERSONA_DOS', 'Nombre de la Persona');	

		// Setea el focus en COD_EMPRESA para las nuevas lineas
		$this->set_first_focus('NOM_PERSONA_DOS');
	}
	function insert_row($row=-1) {
		$row = parent::insert_row($row);
		$this->set_item($row, 'COD_PERSONA_DOS', - $this->redirect($row) - 100);		// Se suma -100 para asegura q que negativo (para el caso 0)
		return $row;
	}
	function fill_record(&$temp, $record) {
		if ($this->cod_empresa){
			$this->controls['P_COD_SUCURSAL_DOS']->retrieve($this->cod_empresa);
		}	
	...
	function update($db){
		...
		for ($i = 0; $i < $this->row_count(); $i++)		{
			...
			$COD_PERSONA = $this->get_item($i, 'COD_PERSONA_DOS');
			$COD_SUCURSAL = $this->get_item($i, 'P_COD_SUCURSAL_DOS');
			
			$NOM_PERSONA = $this->get_item($i, 'NOM_PERSONA_DOS');
			$COD_CARGO = $this->get_item($i, 'COD_CARGO_DOS');
			$TELEFONO = $this->get_item($i, 'TELEFONO_PERSONA_DOS');
			$FAX = $this->get_item($i, 'FAX_PERSONA_DOS');
			$EMAIL = $this->get_item($i, 'EMAIL_DOS');
			$ES_VIGENTE = $this->get_item($i, 'ES_VIGENTE');
			...
			$param = "'$operacion',$COD_PERSONA, $COD_SUCURSAL, '$NOM_PERSONA', $COD_CARGO, $TELEFONO, $FAX, $EMAIL, '$ES_VIGENTE'";
			
			if ($db->EXECUTE_SP($sp, $param)) {
				if ($statuts == K_ROW_NEW_MODIFIED) {
					$COD_PERSONA = $db->GET_IDENTITY();
					$this->set_item($i, 'COD_PERSONA_DOS', $COD_PERSONA);		
				}
			}
			else
				return false;				
		...
		for ($i = 0; $i < $this->row_count('delete'); $i++) {
			...
			$COD_PERSONA = $this->get_item($i, 'COD_PERSONA_DOS', 'delete');
...
class wi_empresa extends wi_empresa_base {
	function wi_empresa($cod_item_menu) {
		...
		$sql = "select COD_EMPRESA,
						...
						,ALIAS_CONTABLE
						,CASE EMP_RELACIONADA_COMISION_TDNX
							WHEN '1' THEN 'S'
							ELSE 'N'
						END EMP_RELACIONADA_COMISION_TDNX	
						,ALIAS_CONTABLE
		...
		*/
		...
		$this->dws['dw_persona_dos'] = new dw_persona_dos();
		...
		$this->dws['dw_empresa']->add_control(new edit_check_box('EMP_RELACIONADA_COMISION_TDNX', 'S', 'N'));
	...
	function new_record() {
		...
		if($priv == 'E')
			$this->dws['dw_empresa']->set_entrable('DSCTO_PERMITIDO', true);	
		else
			$this->dws['dw_empresa']->set_entrable('DSCTO_PERMITIDO', false);
	}
	...
	function load_record() {
		...
		$this->dws['dw_persona_dos']->retrieve($COD_EMPRESA);
		$this->dws['dw_persona_dos']->cod_empresa = $COD_EMPRESA;
		...
		*/
		...
		if($priv == 'E')
			...
			$this->dws['dw_empresa']->set_entrable('DSCTO_PERMITIDO', false);
		$priv = $this->get_privilegio_opcion_usuario('100525', $this->cod_usuario);
		if($priv == 'E'){
        	$this->dws['dw_persona']->set_entrable_dw(false);
        	$this->dws['dw_persona_dos']->set_entrable_dw(true);
        }else{
        	$this->dws['dw_persona']->set_entrable_dw(true);
        	$this->dws['dw_persona_dos']->set_entrable_dw(false);
        }
	...
	function save_record($db) {
		...
		$EMP_RELACIONADA_COMISION_TDNX =$this->dws['dw_empresa']->get_item(0, 'EMP_RELACIONADA_COMISION_TDNX');
		
		if($EMP_RELACIONADA_COMISION_TDNX == 'S')
			$EMP_RELACIONADA_COMISION_TDNX = 1;
		else
			$EMP_RELACIONADA_COMISION_TDNX = 0;	
		...
		// EL CAMPO PORC_DSCTO_CORPORATIVO FUE ELIMINADO DE LA BD	
		$param = "'$operacion',$COD_EMPRESA, $RUT, '$DIG_VERIF', '$ALIAS', '$ALIAS_CONTABLE', '$NOM_EMPRESA', '$GIRO', $COD_CLASIF_EMPRESA, $DIRECCION_INTERNET, $RUT_REPRESENTANTE, $DIG_VERIF_REPRESENTANTE, $NOM_REPRESENTANTE, '$ES_CLIENTE', '$ES_PROVEEDOR_INTERNO', '$ES_PROVEEDOR_EXTERNO', '$ES_PERSONAL', '$IMPRIMIR_EMP_MAS_SUC', '$SUJETO_A_APROBACION', $PORC_DSCTO_CORPORATIVO,$VENDEDOR_CABECERA, $TIPO_PARTICIPACION,$DSCTO_PERMITIDO,$COD_FORMA_PAGO_CLIENTE, $EMP_RELACIONADA_COMISION_TDNX, $MONTO_COMPRA_MAX_SIN_AUTORIZAR";
		...
		if ($db->EXECUTE_SP($sp, $param)){
			...
			for ($i=0; $i<$this->dws['dw_persona_dos']->row_count(); $i++) {
				$COD_SUCURSAL = $this->dws['dw_persona_dos']->get_item($i, 'P_COD_SUCURSAL_DOS');
				if ($COD_SUCURSAL < 0) {
						$row = $this->dws['dw_sucursal']->un_redirect(- $COD_SUCURSAL - 100);		// el -100 viene del insert_row
						$COD_SUCURSAL = $this->dws['dw_sucursal']->get_item($row, 'COD_SUCURSAL');
						$this->dws['dw_persona_dos']->set_item($i, 'P_COD_SUCURSAL_DOS', $COD_SUCURSAL);		
				}
			}
			...
			if (!$this->dws['dw_persona_dos']->update($db))
				return false;	
