...
class wo_cotizacion extends wo_cotizacion_base{
	function wo_cotizacion(){
		...
		$sql = "select	C.COD_COTIZACION
						...
						,C.TOTAL_CON_IVA
						,CASE
							WHEN CHARINDEX('/', dbo.f_get_nro_factura(C.COD_COTIZACION)) = 0 THEN dbo.f_get_nro_factura(C.COD_COTIZACION)
							ELSE SUBSTRING(dbo.f_get_nro_factura(C.COD_COTIZACION), 0, CHARINDEX('/', dbo.f_get_nro_factura(C.COD_COTIZACION))) + '/...'
						END NRO_FACTURA_S
						,dbo.f_get_nro_factura(C.COD_COTIZACION) NRO_FACTURA
				from 		COTIZACION C
							,EMPRESA E
							,USUARIO U
     	...
     	$this->dw->add_control(new edit_text_hidden('NRO_FACTURA'));
	    ...
	    $this->add_header($control = new header_text('NRO_FACTURA', "dbo.f_get_nro_factura(C.COD_COTIZACION)", 'FA'));
