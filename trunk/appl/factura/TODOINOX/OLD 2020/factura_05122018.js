...
function select_1_empresa(valores, record){
	if(valores[1] != '7'){
		...
		ajax.open("GET", "TODOINOX/ajax_get_last_pers.php?cod_empresa="+vl_cod_empresa, false);
		ajax.send(null);
		var resp = ajax.responseText;
		
		if(resp != ''){
			document.getElementById('COD_PERSONA_0').value = resp;
			mail_cargo_persona(document.getElementById('COD_PERSONA_0'));
		}	
	}else{
...
function validate(){
	...
	for(j=0 ; j < vl_aTR.length ; j++){
		if(vl_aTR.length > 0){
			if(document.getElementById('CANTIDAD_'+j)){
