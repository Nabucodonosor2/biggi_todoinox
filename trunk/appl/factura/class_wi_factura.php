...
class dw_factura extends dw_help_empresa {
	...
	function dw_factura() {
		$sql = "SELECT	F.COD_FACTURA,
					...
					,(SELECT top 1 ' / ESTADO OD:'+E.NOM_ESTADO_ORDEN_DESPACHO+' / '+ UO.NOM_USUARIO 
					  ...
					  AND UO.COD_USUARIO = O.COD_USUARIO
					  ORDER BY O.COD_ORDEN_DESPACHO ASC)ORDEN_DESPACHO
					,(SELECT top 1 O.COD_ORDEN_DESPACHO
					  FROM ORDEN_DESPACHO O,ESTADO_ORDEN_DESPACHO E,USUARIO UO 					  
					  WHERE O.COD_DOC_ORIGEN = F.COD_FACTURA
					  AND E.COD_ESTADO_ORDEN_DESPACHO = O.COD_ESTADO_ORDEN_DESPACHO
					  AND UO.COD_USUARIO = O.COD_USUARIO
					  ORDER BY O.COD_ORDEN_DESPACHO ASC) COD_ORDEN_DESPACHO
					 ,NO_TIENE_CC_CLIENTE
		...
		$this->add_control(new static_link('COD_COTIZACION', '../../../../commonlib/trunk/php/link_wi.php?modulo_origen=factura&modulo_destino=cotizacion&cod_modulo_destino=[COD_COTIZACION]&cod_item_menu=1505'));
		...
		$this->add_control(new edit_text_hidden('COD_FACTURA'));
...
class wi_factura_base extends w_cot_nv {
	...
	function enviar_dte($reenviar = false){
		...
		for($i=0 ; $i < $this->dws['dw_referencias']->row_count() ; $i++){
			...
			if($COD_TIPO_REFERENCIA == 5)//802 CAROZZI
				$count_802_car++;
		}
...
/* 				if($ComRecep == ''){
				$this->_load_record();
				print " <script>alert('Error, se detectó item cantidad cero, class_wi_factura linea 2389');</script>";
				return;
			} */
					...
					if(!$reenviar){
						...
						if($WS_ORIGEN == 'BODEGA'){
		           		...
		           		if($sistema <> ''){
							...
							//else if($result_ws == 'HECHO') // ESTE ALERT APARECIA CADA VEZ QUE SE IMPRIME ENVIA DTE NO ES NECESARIO MH 10122020
	...
	function imprimir_dte($es_cedible, $desde_output=false){
		...
		if($cod_factura > 53225)
			...
			if(file_exists("../../../../PDF/PDFTODOINOX/$year/33_$nro_factura.pdf"))
				print " <script>window.open('../../../../PDF/PDFTODOINOX/$year/33_$nro_factura.pdf')</script>";
			else
	...
	function print_od(){
		...
		if ($db->EXECUTE_SP($sp, $param)){
			...
			$sql= "SELECT OD.COD_ORDEN_DESPACHO
					      ...
					      ,ITEM
					      ,LEFT(NOM_PRODUCTO,58) NOM_PRODUCTO
					      ,COD_PRODUCTO
					      ,CANTIDAD
					      ,CANTIDAD_RECIBIDA
					      ,OD.NOM_SUCURSAL
			...
			$rpt = new print_guia_recepcion_fa($sql, $this->root_dir.'appl/orden_despacho/orden_despacho.xml', $labels, "Orden Despacho.pdf", 1);
			$this->_load_record();
			$db->COMMIT_TRANSACTION();
			return true;
		}else{
...
class print_guia_recepcion_fa extends reporte{	
	function print_guia_recepcion_fa($sql, $xml, $labels=array(), $titulo, $con_logo, $vuelve_a_presentacion=false) {
		parent::reporte($sql, $xml, $labels, $titulo, $con_logo, $vuelve_a_presentacion);			
	}
	...
	function modifica_pdf(&$pdf){
		...
		if($result_v[0]['VALIDA'] > 0){
		    ...
		    $pdf->Cell(432, 15, 'Producto', '1', '','C');
		    $pdf->Cell(89, 15, 'Modelo', '1', '','C');
		    $pdf->Cell(30, 15, 'CT', '1', '','C');
