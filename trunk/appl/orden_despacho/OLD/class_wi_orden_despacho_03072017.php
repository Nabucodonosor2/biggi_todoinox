...
class dw_orden_despacho extends dw_help_empresa{
	function dw_orden_despacho(){	
		...
		$this->add_control(new drop_down_dw('RECIBIDO_POR',$sql,145));
...
class wi_orden_despacho extends w_input {
	...
	function load_record() {
		...
		if($cod_estado_orden_despacho == 1){ //EMITIDA
			...
			$this->dws['dw_orden_despacho']->set_entrable('RECIBIDO_POR', false);
			$this->dws['dw_orden_despacho']->set_entrable('ES_RECIBIDO_POR', false);
